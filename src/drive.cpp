...
motor_group ALLdrive = {rgtFrnt, rgtBack, lftBack, lftFrnt};
// Constants ------------------------------------------------------------
...
// Sensor funtions-------------------------------------------------------
bool DriveOff() { return (ALLdrive.isSpinning()); }
...
int getAvg() {
  //  return ((rgtFrnt.position(degrees)) +  (rgtTop.position(degrees)) +
  //  (rgtBot.position(degrees)) + (lftFrnt.position(degrees)) +
  //  (lftTop.position(degrees)) + (lftBot.position(degrees))) /4 ;
...
void Drive(int mspeed, int Distance) {
  ...
  while (1) {
    ...
    LeftDrive(DrivePID(mspeed, Distance) -
              Align()); // sets robot motors to drive forward desired position
                        // and the align keeps it straight
    RightDrive(DrivePID(mspeed, Distance) +
               Align()); // left negetive, right positive
...
int DriveT() {
  ...
  while (1) {
    ...
    double align = oKp * (curHeading() - TurnRor);
